    ...
    for apartment in range(num_apartments):
        ...
        while room_counter <= num_rooms:
            ...
            if room_num in list(map(lambda x: x.room_id, rooms)):
                ...
                continue
            attached_br_input = int(input('enter if attached bathroom. 1 for yes or 0 for no: '))
        ...
        apartments.append(
            Apartment(id=apartment_id, name=apartment_name, address=address, num_rooms=num_rooms, rooms_details=rooms))
...
def _configure_flat_mate():
    while True:
        ...
        print('***********************Below are the apartments configurations********************')
        ...
        room_ref = list(filter(lambda x: x.room_id == room_id, apartment_ref.room_details))[0]
