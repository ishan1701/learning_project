...
from bill import Cleaning, Rental
...
class Apartment:
            ...
            room_details.append(
                {
                    ...
                    "have_bathroom": room.attached_bathroom,
                    "room_mates": room.rooms_mates,
                }
            )
        return room_details
    ...
    def split_bill(self, bill: Union[Rental, Cleaning]):
        if not bill.is_fixed:
            raise ValueError(
                f"Bill {bill.__class__.__name__} should be fixed."
