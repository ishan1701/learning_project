    ...
    def validate(self, explore_content: Dict, views_to_check: List[str]) -> bool:
        ...
        for explore in explore_content.get("explores", []):
            for filter in explore.get("access_filters", []):
                if (
                    filter.get("field") == field
                    and filter.get("user_attribute") == "email"
                ):
                    return True
        return False
...
class FileSeparator:
    ...
    def get_views(files: List[str]) -> List[str]:
        return [
            f for f in files if Path(f).suffix == ".lkml" and "view" in Path(f).stem
        ]

    @staticmethod
    def get_explores(files: List[str]) -> List[str]:
        return [
            f for f in files if Path(f).suffix == ".lkml" and "explore" in Path(f).stem
        ]
...
class ViewCountryValidator(IValidator):
    ...
    def find_views_with_countries(self):
        for f in self.view_files:
            ...
            if self.dim_strategy.has_dimension(
                content["views"][0].get("dimensions", [])
            ):
    ...
    def find_relevant_explores(self):
        view_names = [
            Path(f).stem.replace(".view", "") for f in self.views_with_countries
        ]
        for explore_file in self.reader.scan_all_explores():
            content = self.reader.read(
                explore_file.relative_to(self.reader.project_root).as_posix()
            )
        ...
        for explore_file in self.explores_to_validate:
            ...
            for strategy in self.explore_strategies:
                if not strategy.validate(content, self.views_with_countries):
                    raise KeyError(
                        f"Validation failed for {explore_file} with {strategy.__class__.__name__}"
                    )
...
class ExploresCountryValidator(IValidator):
    ...
    def find_views_from_explores(self):
        for f in self.explore_files:
            ...
            for explore in content.get("explores", []):
                ...
                view_path = (
                    self.reader.project_root / "views" / f"{view_name}.view.lkml"
                )
                if view_path.exists():
    ...
    def check_views_for_dimensions(self):
        for f in self.view_files_to_check:
            content = self.reader.read(
                Path(f).relative_to(self.reader.project_root).as_posix()
            )
            if self.dim_strategy.has_dimension(
                content["views"][0].get("dimensions", [])
            ):
                raise ValueError(
                    f"Country dimension found in {f} without access filter"
                )
...
class Validate:
    def __init__(self, files: List[str]):
        ...
        explore_strategies = [
            ...
            AccessFilterValidationStrategy(),
        ]

        self.validators = [
            ViewCountryValidator(files, reader, dim_strategy, explore_strategies),
            ExploresCountryValidator(
                files, reader, dim_strategy, AccessFilterValidationStrategy()
            ),
        ]
