...
from components.processing_unit import IntelCPU, CPU
...
if __name__ == '__main__':
    ...
    product = 'gaming_pc'
    keyboard: Keyboard = DellKeyboardRechargeable(model='xxs3', type='qwerty', is_multilingual=True,
                                                  battery_capacity=10)
    memory: RAM = DDR3(capacity_in_gb=10, manufacturer='DDR3_manufacturer', type='very_volaile', speed_range=22,
                       latency=0.33)

    storage: Storage = SSD(read_speed_mb=122.22, write_speed_mb=12.1, capacity=1024, interface='ssd_interface')

    cpu: CPU = IntelCPU(model='pentium 22', core=13, cache_mb=1024)

    product = main(product, memory=memory, storage=storage, cpu=cpu, keyboard=keyboard,graphics_card_memory=1234)
