...
def _create_spark_conf(
    app_name: str, config: dict, open_table_format, catalog_type: str = "nessie"
) -> SparkConf:
    """create spark conf"""
...
def get_pyspark_session(config: dict, app_name, open_table_format) -> SparkSession:
    ...
    spark_conf = _create_spark_conf(
        app_name=app_name, config=config, open_table_format=open_table_format
    )
