...
def get_bq_client(credential_handler: CredentialHandler, project_id: str) -> bigquery.client:
...
class GCSBQFileLoader(ABC):
    @staticmethod
    @abstractmethod
    def load_file(gcs_path, credential_handler: CredentialHandler, bq_project_id, bq_table_str, bq_dataset: str,
                  **kwargs):
        pass
...
class CSVFileLoader(GCSBQFileLoader):
    ...
    @staticmethod
    def load_file(gcs_path, credential_handler: CredentialHandler, bq_project_id: str, bq_table: str, bq_dataset: str,
                  **kwargs):
        # skip_leading_rows = 1 if not 'skip_first_row' in kwargs else 0

        bq_client = get_bq_client(credential_handler=credential_handler, project_id=bq_project_id)
        ...
        bq_client.load_table_from_uri(source_uris=gcs_path, destination=f'{bq_project_id}.{bq_dataset}.{bq_table}',
...
class FileLoader:
    ...
    def load_file(self, gcs_path, credential_handler: CredentialHandler, bq_project_id: str, bq_table: str,
                  bq_dataset: str, **kwargs):
        self.file_loader.load_file(gcs_path, credential_handler, bq_project_id, bq_table, bq_dataset, **kwargs)
